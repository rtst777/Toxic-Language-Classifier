...
class Glove_Based_LSTM_Model(nn.Module):
    def __init__(self, index_to_vocab=None, input_size=INPUT_SIZE, hidden_size=INPUT_SIZE, num_classes=3):
        super(Glove_Based_LSTM_Model, self).__init__()
        self.index_to_vocab = index_to_vocab
        self.glove = torchtext.vocab.GloVe(name="6B", dim=INPUT_SIZE)  # TODO tune number
    ...
    def convert_input_to_glove(self, x):
        if isinstance(x, list):
            out = torch.zeros(1, len(x), INPUT_SIZE)
            for i in range(len(x)):
                out[0][i] = self.glove[x[i]]
        else:
            out = torch.zeros(x.shape[0], x.shape[1], INPUT_SIZE)
